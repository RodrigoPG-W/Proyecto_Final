...
#include <windows.h>			//Función cls y goto.
...
nodo *HEAD;						//Apuntador al primer (HEAD) y último (TAIL) elemento. Iniciados en NULL pues no existe la lista.
nodo *TAIL;
								//Funciones del menú.
...
int BusquedaCompleta;		//Permite repetir, Almacena el nombre del archivo, Auxiliar para mantener el nombre de un archivo, Almacena parámetro de búsqueda, Auxiliar para verificación de estado de búsqueda

int main(){
	char Iniciar='1';					//Auxiliar para repetir el programa
	char Opcion,OpcionArchivo;			//Auxiliares para la selección en los menús.
	while (Iniciar=='1')
		{
		HEAD = NULL;						//Inicia la lista. En caso de repetir, reinicia.
		TAIL = NULL;
		r='1';								//Auxiliar para repetición.
		printf("%cBienvenid@!\n\n",173);
				...
				switch(OpcionArchivo)
					{
						...
						case 'D':
							Opcion='0';
							system("cls");
							Ayuda(Opcion);					//Muestra ayuda dependiendo del menú en el que se encuentre.
							r='1';
							break;
						default:
							system("cls");
		...
		while(Opcion != 'F')								//Permite realizar múltiples tareas sobre el inventario antes de guardar.
			{
				gotoxy(5,1);								//Desplaza y mantiene el menú en una sola coordenada
				Menu();
				scanf("%s", &Opcion);
...
void Buscar(){
	int x;									//Auxiliar para función strcmpy.
...
void Modificar(){
	int x;													//Auxiliar para función strcmpy.
	char r='1',r2,OpcionM;									//Auxiliares para elecciones de modificación.
	while (r=='1')											//Permite repetir el proceso hasta que el usuario decida.
		{
			nodo *actual = (nodo*) malloc(sizeof(nodo));	//Creación de un apuntador para recorrer la lista.
			actual = HEAD;									//Se inicia por HEAD.
...
void Mostrar(){
	...
	int puntosMenos,i,longitudM;							//Auxiliares para la impresión
	nodo *actual = (nodo*) malloc(sizeof(nodo));			//Creación de un apuntador para recorrer la lista.
	actual = HEAD;											//Se inicia por HEAD.
	if(HEAD!=NULL)											//Comprobación de que la lista no esté vacía.
		{
			...
			while(actual != NULL)							//Condición para recorrer hasta el TAIL de la lista.
				{
					puntosMenos=0;							//Inicializa en 0 para cada repetición del ciclo, pues se imprimen n productos.
					printf("\n %s",actual->item);			//Muestra producto.
					longitudM=strlen(actual->item);			//Obtiene la longitud del producto para reservar el espacio.
					for (i=1;i<=(20-longitudM);i++)			//Tiene predefinido imprimir 20 espacios, a los cuales les será restado la cantidad reservada.
						{									//Imprimiendo puntos en su lugar para mantener la estética.
							printf(".");	
						}
					...
					if (longitudM<0)
						{
							puntosMenos++;					//Si el inventario contiene un número negativo, reserva el espacio.
						}
					while ((longitudM/10)!=0)
						{
							longitudM=(longitudM/10);		//Divide entre 10 para conocer el tamaño del número, pues al llegar al cero implica que...
							puntosMenos++;					//se han reservado suficientes espacios, pues se le suma uno en cada repetición.
						}
	...
	if (menuayuda=='0')
		{
			...
			printf("B) Permite elegir un archivo generado previamente para cargar los nodos del mismo, poniendo el nombre del archivo (sin extensi%cn). El archivo debe estar en la misma ubicación que su programa.\n\n",162);
...
void gotoxy(int xc, int yc){			//Función impartida en clase de Fundamentos de Programación por Cintia Quezada.
	COORD coord;						//Asigna coordenadas en pantalla para la impresión (sin embargo, la coordenada x no funciona correctamente en el SO Windows 10).
	coord.X=xc;							
	coord.Y=yc;
